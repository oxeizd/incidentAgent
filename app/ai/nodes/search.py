...
Wave 1: перешли с result.tool_results (flatten) на result.artifacts() —
tool artifacts (SearchPage) теперь имеют стабильный контракт:
- total_count — точное число совпадений до pagination;
- returned_count / items — текущая страница сырых записей;
- has_more — есть ли следующая страница;
- display — поля для чата из data/config/search_output.yaml.

Итоговый summary НЕ использует len(...) как количество найденных
инцидентов/поручений — это было бы числом записей на одной странице тула,
а не точным total_count по всем совпадениям.
"""
...
async def search_node(ctx: NodeCtx) -> dict:
    ...
    artifacts = [artifact for artifact in result.artifacts() if _is_search_page(artifact)]

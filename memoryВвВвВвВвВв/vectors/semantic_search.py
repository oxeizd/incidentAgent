...
class SemanticSearchService:
    """
    Generic semantic nearest-neighbour retrieval.

    Global semantic query uses sqlite-vec KNN. Hybrid search supplies
    allowed_ids after structured filtering and reranks vectors only within
    this permitted candidate set.
    """
        ...
        *,
        ...
        self._entities: dict[EntityType, SemanticEntityConfig] = {
            "incidents": SemanticEntityConfig(
                entity="incidents",
                vector_source=(
                    "description, reason_inc, solution, "
                    "resolution_description, impact, ai_description"
                ),
            ...
            "assignments": SemanticEntityConfig(
                entity="assignments",
                vector_source="assignment, task, unit",
    ...
    async def search(
        ...
        *,
        ...
        config = self._get_config(entity)
    ...
    def _get_config(
        ...
        try:
            ...
            raise ValueError(
                f"Unsupported semantic entity: {entity!r}"
            ) from exc
...
def _filter_and_rank_knn_matches(
    *,
    ...
    """
    sqlite-vec returns L2 distance. Unit-normalized vectors allow:

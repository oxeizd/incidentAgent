...
import type { AgentEventEnvelope, AgentFailureData, SseEvent, SseMessagePayload, ToolCall } from "./types";
import {
  addErrorMessage,
  addMessage,
  getMessagesEl,
...
// Agent activity is ephemeral UI state. It is never appended to chat history.
// Reasoning/status events can arrive frequently, therefore DOM updates are
// coalesced to one per browser animation frame.
let pendingActivity: { threadId: string; message: string } | null = null;
...
function scheduleActivity(targetThreadId: string, message: string): void {
  pendingActivity = { threadId: targetThreadId, message };
  ...
  activityFrame = requestAnimationFrame(() => {
    ...
    showTyping(activity.message);
  });
}

function clearActivity(targetThreadId: string): void {
  if (pendingActivity?.threadId === targetThreadId) pendingActivity = null;
...
async function submitToServer(targetThreadId: string, opts: SubmitOptions): Promise<void> {
  ...
  if (targetThreadId === threadId) {
    showTyping("Агент начинает обработку…");
    updateSendButton();
  }
...
function progressMessage(data: unknown): string | null {
  if (!data || typeof data !== "object") return null;
  const message = (data as { message?: unknown }).message;
  return typeof message === "string" && message.trim() ? message : null;
}

function finalPayload(event: SseEvent): SseMessagePayload | null {
  if (event.event === "message") return event.data;
  if (event.event === "message.final") return event.data.data as SseMessagePayload;
  return null;
}

function failurePayload(event: SseEvent): AgentFailureData | null {
  if (event.event === "error") return event.data;
  if (event.event === "run.failed") return event.data.data as AgentFailureData;
  if (event.event === "tool.failed") return event.data.data as AgentFailureData;
  return null;
}

function handleSseEvent(targetThreadId: string, event: SseEvent): void {
  if (event.event === "agent.status" || event.event === "agent.reasoning" || event.event === "reasoning") {
    const envelope = event.data as AgentEventEnvelope;
    if (envelope.visibility === "developer") return;
    const message = progressMessage(envelope.data);
    if (message && targetThreadId === threadId) scheduleActivity(targetThreadId, message);
    return;
  }

  const failure = failurePayload(event);
  if (failure) {
    clearActivity(targetThreadId);
    if (targetThreadId === threadId) addErrorMessage(failure.message);
    return;
  }

  const final = finalPayload(event);
  if (final) {
    clearActivity(targetThreadId);
    if (final.awaiting_input && final.tool_calls?.length) {
      if (targetThreadId === threadId) setPendingInterrupt(targetThreadId, final.tool_calls[0]);
      return;
    }

    if (targetThreadId === threadId) clearPendingInterrupt();
    if (final.content && targetThreadId === threadId) {
      addMessage(targetThreadId, "assistant", final.content, final.artifact);
    }
    return;
  }
...
async function loadMessages(): Promise<void> {
  ...
  try {
    ...
    if (pendingControllers.has(targetThreadId)) showTyping("Агент продолжает обработку…");
  } catch (error) {
    if (threadId !== targetThreadId) return;
...
async function handleSendClick(): Promise<void> {
  ...
  try {
    ...
    if ((error as Error).name !== "AbortError" && targetThreadId === threadId) {
      clearActivity(targetThreadId);
      addErrorMessage(error instanceof ApiError ? error.message : "Не удалось отправить сообщение. Попробуйте ещё раз.");
    }
  }
}

...
def render_presentation(document: PresentationDocument) -> str:
    """
    Render a self-contained HTML presentation from one stored document.

    The renderer has no database, LLM, workflow or HTTP dependencies. The
    exact same document always produces the same presentation structure.
    """
    tabs = _tabs(document)
    panels = [
        _summary_panel(document),
        _incident_panel(document),
        _analysis_panel(document),
        *[
            _assignment_panel(assignment, index)
            for index, assignment in enumerate(document.assignments, start=1)
        ],
    ]
...
<head>
  ...
  <title>{_text(document.brief)} — презентация</title>
  <style>
    :root {{
      --bg-top: #f3f3f1;
      --bg-bottom: #d9def7;
    ...
    * {{ box-sizing: border-box; }}
    body {{ margin: 0; min-width: 1060px; background: #e7eaf4; color: var(--text); }}
    ...
    .vp-root {{
      ...
      border-radius: 16px;
      background: linear-gradient(180deg, var(--bg-top) 36%, var(--bg-bottom) 100%);
    ...
    .vp-tribe {{ font-size: 15px; color: #5e5a56; margin-bottom: 5px; }}
    .vp-title {{ margin: 0; font-size: 27px; font-weight: 700; }}
    ...
    .vp-nav.visible {{ display: flex; }}
    .vp-nav.left {{ margin-right: 5px; }}
    .vp-nav.right {{ margin-left: 5px; }}
    ...
    .vp-summary-table tr:last-child td {{ border-bottom: 0; }}
    .clickable-row {{ cursor: pointer; }}
    .clickable-row:hover {{ background: rgba(152, 170, 252, .12); }}
    .vp-arrow {{ color: #5367ca; font-size: 22px; text-align: center; }}
    ...
    .vp-info-value {{ font-size: 15px; word-break: break-word; }}
    ...
    .vp-chain.first {{ border-top: none; }}
    ...
    .vp-measure-title {{ display: block; font-weight: 650; }}
    .vp-measure-note {{
      display: block;
      margin-top: 3px;
      color: #6c7480;
    ...
    .vp-alert:first-child {{ border-top: 0; }}
    .vp-alert-mark {{ height: 21px; border-radius: 4px; }}
    .vp-alert-mark.ok {{ background: var(--good); }}
    .vp-alert-mark.bad {{ background: var(--bad); }}
    .vp-alert-status {{ color: #6c7480; font-size: 13px; }}

    .vp-loss {{ padding: 16px; }}
    .vp-loss-label {{ font-size: 16px; font-weight: 700; margin-bottom: 8px; }}
    .vp-loss-value {{ font-size: 34px; color: #64748b; }}

    .vp-timeline {{
      min-height: 310px;
      background: linear-gradient(180deg, var(--timeline), #24323a);
      color: #fff;
      border-radius: 14px;
      padding: 15px;
    }}
    ...
    .assignment-value.empty {{ color: #64748b; }}
...
def _tabs(document: PresentationDocument) -> str:
    tabs = [
        ...
        _tab("incident", f"Инцидент {_text(document.number)}"),
        _tab("analysis", "RCA-анализ"),
    ]
    ...
    tabs.extend(
        ...
        for index, _ in enumerate(document.assignments, start=1)
    )
...
def _summary_panel(document: PresentationDocument) -> str:
    assignment_rows = "".join(
        ...
        for index, assignment in enumerate(document.assignments, start=1)
    ) or _empty_row("Нет связанных поручений.")
...
<section class="vp-panel active" data-panel="summary">
  ...
  <div class="vp-summary-table-wrap">
    <table class="vp-summary-table">
      ...
      <tbody>
        <tr class="clickable-row" data-open-tab="incident">
          <td>{_text(document.number)}</td>
          <td>{_core_system(document.chain)}</td>
          <td>{_text(document.brief)}</td>
          <td>{_responsible(document.team)}</td>
          <td>{_text(document.losses)}</td>
...
def _incident_panel(document: PresentationDocument) -> str:
    system_measures = "".join(
        _assignment_measure_row(assignment, index)
        for index, assignment in enumerate(document.assignments, start=1)
    ) or _measure_row(document.systemic_measures, "—")

    return f"""
<section class="vp-panel" data-panel="incident">
  <div class="vp-grid">
    <div class="vp-left">
      <div class="vp-info-strip">
        {_info("ИОР", document.number)}
        {_info("Команда", _team_part(document.team))}
        {_info("Юнит", document.unit)}
        {_info("Этап процесса", document.stage)}
      </div>

      {_card("Описание", _text(document.description))}
      {_card("Причина", _text(document.cause))}
      {_chain_card(document.chain)}
      {_card("Влияние на продукт", _text(document.impact))}
      {_alerts_card(document)}
      {_measures_card(
          "Оперативные мероприятия",
          _measure_row(document.operational_measures, "—"),
      )}
    ...
    <aside class="vp-right">
      <article class="vp-loss">
        ...
        <div class="vp-loss-value">{_text(document.losses)}</div>
      </article>

      {_timeline(document.timeline)}
    </aside>
  </div>
</section>
"""
...
def _analysis_panel(document: PresentationDocument) -> str:
    analysis = _text(document.analysis_markdown)

    return f"""
<section class="vp-panel" data-panel="analysis">
  <article class="vp-card">
    <div class="vp-card-title">RCA-анализ</div>
    <div class="vp-analysis">{analysis}</div>
  </article>
</section>
"""
...
<section class="vp-panel" data-panel="assignment-{index}">
  <div class="assignment-layout">
    <article class="vp-card">
      ...
      {_assignment_detail("Ожидаемый результат", assignment.expected_result)}
      {_assignment_detail(
...
def _assignment_measure_row(
    assignment: PresentationAssignment,
    index: int,
) -> str:
    deadline = _join_nonempty(
        [
            assignment.responsible,
            _deadline_label(assignment.deadline_at),
        ]
    )

    return f"""
<button
  class="vp-mrow clickable"
  data-open-tab="assignment-{index}"
  type="button"
>
  <span>
    <span class="vp-measure-title">{_text(assignment.title)}</span>
    <span class="vp-measure-note">
      Устраняет: {_text(assignment.addresses)}
    </span>
  </span>
  <span>{_text(deadline)}</span>
</button>
"""


def _measure_row(measure: str, responsible: str) -> str:
    return f"""
<div class="vp-mrow">
...
def _measures_card(title: str, rows: str) -> str:
    return f"""
<article class="vp-card">
...
def _alerts_card(document: PresentationDocument) -> str:
    rows = [
        _alert_row(alert.text, "Сработал", "ok")
        for alert in document.alerts.worked
    ]
    rows.extend(
        _alert_row(alert.text, "Не сработал", "bad")
        for alert in document.alerts.failed
    )
...
def _alert_row(text: str, status: str, marker: str) -> str:
    return f"""
<div class="vp-alert">
...
def _split_timeline_event(event: str) -> tuple[str, str]:
    ...
    for separator in (" — ", " – ", " - "):
        if separator in normalized:
            timestamp, description = normalized.split(separator, 1)
            return timestamp.strip(), description.strip()

    return "", normalized


def _info(label: str, value: str | None) -> str:
    return f"""
<div class="vp-info">
...
def _meta(label: str, value: str | None) -> str:
    return f"""
<div class="meta">
...
def _core_system(chain: str) -> str:
    for line in chain.splitlines():
        ...
        if normalized.casefold().startswith("корневой:"):
            return normalized.split(":", 1)[1].strip() or "—"

    return chain.splitlines()[0].strip() if chain.splitlines() else "—"


def _team_part(team: str) -> str:
    return team.split(",", 1)[0].strip() or "—"


def _responsible(team: str) -> str:
    parts = [
        part.strip()
        for part in team.split(",")[1:]
        if part.strip()
    ]
...
def _deadline_label(value: str | None) -> str | None:
    if not value:
        return None

    try:
        return f"до {datetime.fromisoformat(value.replace('Z', '+00:00')).strftime('%d.%m.%Y')}"
    except ValueError:
        return value
